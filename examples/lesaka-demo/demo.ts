import {
  clearCache,
  click,
  highlight,
  moveTo,
  tooltip,
  type,
  wait,
  waitForNavigation,
  walkr,
} from "@walkrstudio/core";

/**
 * Lesaka Insights — Sign-in walkthrough.
 *
 * Flow: Visit login page -> enter email -> enter password -> sign in.
 */
export default walkr({
  url: "http://localhost:5173/login",
  title: "Lesaka Insights — Sign In",
  description: "Walk through the sign-in flow for Lesaka Insights.",

  viewport: { width: 1366, height: 768 },

  cursor: {
    shape: "svg",
    color: "#c8ee44",
    size: 24,
    shadow: true,
    clickColor: "#a8cc22",
    svgContent:
      '<svg viewBox="0 0 24 24" fill="#ffffff" xmlns="http://www.w3.org/2000/svg"><path d="M17.2607 12.4008C19.3774 11.2626 20.4357 10.6935 20.7035 10.0084C20.9359 9.41393 20.8705 8.74423 20.5276 8.20587C20.1324 7.58551 18.984 7.23176 16.6872 6.52425L8.00612 3.85014C6.06819 3.25318 5.09923 2.95471 4.45846 3.19669C3.90068 3.40733 3.46597 3.85584 3.27285 4.41993C3.051 5.06794 3.3796 6.02711 4.03681 7.94545L6.94793 16.4429C7.75632 18.8025 8.16052 19.9824 8.80519 20.3574C9.36428 20.6826 10.0461 20.7174 10.6354 20.4507C11.3149 20.1432 11.837 19.0106 12.8813 16.7454L13.6528 15.0719C13.819 14.7113 13.9021 14.531 14.0159 14.3736C14.1168 14.2338 14.2354 14.1078 14.3686 13.9984C14.5188 13.8752 14.6936 13.7812 15.0433 13.5932L17.2607 12.4008Z" stroke="#c8ee44" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
    offset: { x: 0.15, y: 0.1 },
  },

  steps: [
    // ── Clear browser state so login page appears fresh ──
    clearCache(),
    wait(800),

    // ── Scene 1: Enter email ──────────────────────────────────────
    tooltip('[data-testid="email-input"]', "Enter your email address", {
      position: "top",
      duration: 1500,
    }),
    moveTo('[data-testid="email-input"]', { duration: 600 }),
    click('[data-testid="email-input"]'),
    type("admin@lesaka.dev", { selector: '[data-testid="email-input"]', delay: 40 }),

    wait(300),

    // ── Scene 2: Enter password ───────────────────────────────────
    tooltip('[data-testid="password-input"]', "Now enter your password", {
      position: "top",
      duration: 1500,
    }),
    moveTo('[data-testid="password-input"]', { duration: 400 }),
    click('[data-testid="password-input"]'),
    type("admin123!", { selector: '[data-testid="password-input"]', delay: 50 }),

    wait(400),

    // ── Scene 3: Sign in ──────────────────────────────────────────
    moveTo('[data-testid="login-btn"]', { duration: 400 }),
    highlight('[data-testid="login-btn"]', {
      spotlight: true,
      color: "#c8ee44",
      duration: 1000,
      backdropOpacity: 0.3,
      padding: 8,
    }),
    click('[data-testid="login-btn"]'),

    // Wait for navigation to complete
    waitForNavigation({ waitUntil: "networkidle" }),
    wait(1500),
  ],
});
