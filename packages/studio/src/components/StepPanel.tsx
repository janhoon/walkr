    ...
    <div style={panelStyle}>
      ...
      <div style={fieldGroupStyle}>
        ...
        <input
          ...
          value={step.duration}
          min={0}
          onChange={(e) => updateDuration(Math.max(0, parseInt(e.target.value) || 0))}
        />
      </div>
    ...
    case 'type':
      return (
        <>
          ...
          <NumberField label="Delay (ms per char)" value={(opts.delay as number) ?? 0} onChange={(v) => updateOptions({ delay: v })} />
    ...
    case 'scroll':
      return (
        <>
          <NumberField label="X" value={opts.x as number} onChange={(v) => updateOptions({ x: v })} />
          <NumberField label="Y" value={opts.y as number} onChange={(v) => updateOptions({ y: v })} />
          <CheckboxField label="Smooth" checked={!!opts.smooth} onChange={(v) => updateOptions({ smooth: v })} />
        </>
      )

    case 'wait':
      return (
        <NumberField label="Wait (ms)" value={opts.ms as number} onChange={(v) => updateOptions({ ms: v })} />
      )

    case 'zoom':
      return (
        <>
          <NumberField label="Level" value={opts.level as number} onChange={(v) => updateOptions({ level: v })} min={0.5} max={4} step={0.1} />
          <CheckboxField label="Follow" checked={!!opts.follow} onChange={(v) => updateOptions({ follow: v })} />
          <SelectField label="Easing" value={(opts.easing as string) ?? 'linear'} options={EASING_OPTIONS} onChange={(v) => updateOptions({ easing: v })} />
        </>
      )
    ...
    case 'highlight':
      return (
        <>
          ...
          <NumberField label="Duration (ms)" value={(opts.duration as number) ?? 500} onChange={(v) => updateOptions({ duration: v })} />
        </>
      )
